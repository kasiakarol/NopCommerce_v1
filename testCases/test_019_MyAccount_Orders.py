...
class Test_019_MyAccountOrders():
    ...
    def test_orders(self, setup):
        ...
        self.orders = self.myor.order_list()

        if self.order.title() in self.orders:
            assert True
            self.driver.close()
        else:
            self.driver.save_screenshot(os.path.abspath(os.curdir) + "\\screenshots\\" + "test_myaccount_orders.png")
            self.driver.close()
            assert False
